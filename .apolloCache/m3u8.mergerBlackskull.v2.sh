...
sleep 3
busybox echo -e "#!$SHELL\n## Auto generated by $(basename $0) at $(date +%d/%m/%Y) $(date +%T) ##" > $black/00-$(basename $0)
...
busybox echo -e "#!$SHELL\n## Auto generated by $(basename $0) at $(date +%d/%m/%Y) $(date +%T) ##\nbusybox ls $black/ts/*.ts > $black/allts.log\necho \"Running \$(basename \$0)....\"\nblackskullAuVid ()\n{\necho \"M3U8 to mkv\"\n}\nblackskullAuVid\nsleep 4\ncat $black/allts.log|busybox awk '{print \"file \\\x27\" \$0 \"\\\x27\"}' > $black/myFile.txt\nffmpeg -f concat -safe 0 -i $black/myFile.txt -c copy \"$black/$1.mkv\"\nsleep 2\nrm $black/ts/*.ts" > $black/01-$(basename $0)
